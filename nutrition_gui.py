...
class Nutrition_Gui:
    def __init__(self, day):
        ...
        # Daily nutrient data
        self.groups = day.groups
        self.nutrients = day.nutrients
        # DISPLAY GUI
        self.setup()
    ...
    def hover_box(self, event):
        ...
        for nutrient in nutrients:
            ...
            self.info_labels[nutrients.index(nutrient)].grid(row=nutrients.index(nutrient),column=len(self.nutrients))
    def setup(self):
        i = 0
        for group in self.nutrients:
            self.nutrient_labels.append(Label(self.master,text=self.groups[self.nutrients.index(group)]).grid(row=0,column=self.nutrients.index(group)))
            ...
            for nutrient in group:
                ...
                self.nutrient_labels[i].grid(row=group.index(nutrient)+1,column=self.nutrients.index(group))
